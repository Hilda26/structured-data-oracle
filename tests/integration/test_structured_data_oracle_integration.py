...
@pytest.mark.integration
def test_check_feed_reaches_condition_not_met_on_a_real_impossible_threshold(
    deployed_contract, creator_account, checker_account
):
    """
    Proves the negative path works on real live consensus, not just
    CONDITION_MET: the same real API, but a threshold Bitcoin's price can
    never satisfy (< 1). The judged round must still find the real value
    and correctly report CONDITION_NOT_MET - not NOT_FOUND, since the
    value genuinely was found, it just fails the comparator.
    """
    c = deployed_contract
    creator = c.connect(creator_account)
    checker = c.connect(checker_account)
    ...
    check_result = checker.check_feed(args=[feed_id]).transact(**SLOW_WAIT)
    print("impossible-threshold check_feed receipt:", check_result)
    assert tx_execution_succeeded(check_result), (
        "check_feed failed or returned UNDETERMINED - known retryable "
        "StudioNet behavior; rerun this test if so"
    )
    checked = c.get_feed(args=[feed_id]).call()
    print("checked feed (impossible threshold):", checked)
    assert checked["state"] == "CONDITION_NOT_MET", checked
    assert checked["extracted_value"] != "", "the value must have been found to correctly fail the comparator"


@pytest.mark.integration
def test_check_feed_with_unreachable_api_completes_without_genvm_or_consensus_error(
    deployed_contract, creator_account, checker_account
):
    """
    A genuinely dead domain for the api_url - proves a real fetch failure
    is absorbed as a contract-level ERRORED state, never a GenVM execution
    error or a failed/undetermined consensus round."""
    c = deployed_contract
    creator = c.connect(creator_account)
    checker = c.connect(checker_account)

    dead_url = "https://this-domain-does-not-exist-oracle999xyz.com/api"
    create_result = creator.create_feed(args=[dead_url, FIELD_DESC, ">", "1", COOLDOWN]).transact(**FAST_WAIT)
    assert tx_execution_succeeded(create_result), create_result
    feed_id = int(c.feed_count(args=[]).call()) - 1

    check_result = checker.check_feed(args=[feed_id]).transact(**SLOW_WAIT)
    print("unreachable-api check_feed receipt:", check_result)
    assert tx_execution_succeeded(check_result), (
        "check_feed must succeed at the consensus/GenVM level even when "
        "the declared API is genuinely unreachable"
    )
    checked = c.get_feed(args=[feed_id]).call()
    print("checked feed (unreachable API):", checked)
    assert checked["state"] == "ERRORED", checked
    assert checked["extracted_value"] == ""
